...
func TestMaxPendingTotal(t *testing.T) {
	c1 := certificatesv1beta1.CertificateSigningRequest{
	...
	csrList1 := []certificatesv1beta1.CertificateSigningRequest{
		c1,
	}
	csrList2 := []certificatesv1beta1.CertificateSigningRequest{}

	csrList3 := []certificatesv1beta1.CertificateSigningRequest{
		c1,
		c1,
		c1,
		c1,
	...
	ml1 := []machinev1beta1.Machine{}
	ml2 := []machinev1beta1.Machine{
		{
			Status: machinev1beta1.MachineStatus{
				NodeRef: &corev1.ObjectReference{
					Name: "test",
	...
	for i, tc := range testCases {
		mpt := maxPendingTotal(tc.csrs, tc.ml, tc.maxPendingCSRs)
		if mpt != tc.expected {
			t.Errorf("test case %v failed; found: %v; expected: %v", i, mpt, tc.expected)
		}
	}
}
