...
var (
	...
	webconsoleBump = regexp.MustCompile(regexp.QuoteMeta("bump(github.com/openshift/service-serving-cert-signer-web-console): ") + `([\w]+)`)
...
func main() {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		// has api changes
		display := fmt.Sprintf("%s [\\#%s](https://github.com/openshift/cluster-machine-approver/pull/%s)", message, matches[1], matches[1])
