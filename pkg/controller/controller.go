...
func (m *CertificateApprover) buildWithManager(mgr ctrl.Manager, options controller.Options, c reconcile.Reconciler) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			...
			&handler.EnqueueRequestsFromMapFunc{ToRequests: handler.ToRequestsFunc(m.toCSRs)},
...
func (m *CertificateApprover) toCSRs(handler.MapObject) []reconcile.Request {
...
func (m *CertificateApprover) Reconcile(req ctrl.Request) (reconcile.Result, error) {
	csrs := &certificatesv1.CertificateSigningRequestList{}
	if err := m.List(context.Background(), csrs); err != nil {
		return reconcile.Result{}, fmt.Errorf("Failed to get CSRs: %w", err)
	}

	machines := &machinev1.MachineList{}
	if err := m.List(context.Background(), machines); err != nil {
	...
	for _, csr := range csrs.Items {
		if csr.Name == req.Name && csr.Namespace == req.Namespace {
			return reconcile.Result{}, m.reconcileCSR(csr, machines)
		}
	}

	return reconcile.Result{}, fmt.Errorf("Failed to find CSR: %v", req)
}
